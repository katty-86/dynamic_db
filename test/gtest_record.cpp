...
TEST_F(RecordTest, updateRecord){
	...
	vps.push_back(std::pair<std::string, std::string>("1111111", ""));
	record->addRecord(vec_config, vps);
	EXPECT_FALSE(record->updateRecord("22.0", 1,  Data_type::INT));
	EXPECT_FALSE(record->updateRecord("ssss", 0,  Data_type::INT));
	EXPECT_FALSE(record->updateRecord("ssss", 0,  Data_type::FLOAT));

	record->updateRecord("22", 0,  Data_type::INT);
	std::string record_s =" 22; SSSS; 13.2; 1111111;";
	std::ostringstream oss;
	oss<<*(record);
	EXPECT_STREQ(record_s.c_str(), (oss.str()).c_str());

	record->updateRecord("22", 1,  Data_type::VAR);
	record_s =" 22; 22; 13.2; 1111111;";
	oss.clear();
	oss.str("");
	oss<<*(record);
	EXPECT_STREQ(record_s.c_str(), (oss.str()).c_str());

	record->updateRecord("22.0", 2,  Data_type::FLOAT);
	record_s =" 22; 22; 22; 1111111;";
	oss.clear();
	oss.str("");
	oss<<*(record);
	EXPECT_STREQ(record_s.c_str(), (oss.str()).c_str());

	record->updateRecord("22", 3,  Data_type::TIME);
